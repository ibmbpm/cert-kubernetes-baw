...
function prompt_license(){
    ...
    echo -e "\x1B[1;31mhttps://www.ibm.com/support/customer/csol/terms/?id=L-BKWX-VWBQKW&lc=en\n\x1B[0m"
        ...
        if [[ $tmp_init_flag == "Yes" || $tmp_init_flag == "YES" || $tmp_init_flag == "Y" || $tmp_init_flag == "True" || $tmp_init_flag == "true" ]]; then
            ...
            if [[ "$SCRIPT_MODE" == "dev" ]]; then
                ...
                if grep -q "bai_configuration:" ${CP4A_PATTERN_FILE_TMP}; then
                    ...
                    ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.shared_configuration.images.keytool_init_container.tag "$CURRENT_SPRINT_TAG"
                fi
            fi
    ...
    if [[ "$SCRIPT_MODE" == "dev" ]]; then
        ...
        if grep -q "bai_configuration:" ${CP4A_PATTERN_FILE_TMP}; then
            ...
            if ! grep -q "cp.stg.icr.io/cp/cp4a/bai" ${CP4A_PATTERN_FILE_TMP}; then
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.business_performance_center.image.tag "$CURRENT_SPRINT_TAG"
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.content.image.tag "$CURRENT_SPRINT_TAG"
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.icm.image.tag "$CURRENT_SPRINT_TAG"
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.init_image.image.tag "$CURRENT_SPRINT_TAG"
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.management.backend.image.tag "$CURRENT_SPRINT_TAG"
                ...
                ${YQ_CMD} w -i ${CP4A_PATTERN_FILE_TMP} spec.bai_configuration.management.image.tag "$CURRENT_SPRINT_TAG"
