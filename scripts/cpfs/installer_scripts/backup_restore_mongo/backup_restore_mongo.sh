...
function prereq() {
    ...
    if [[ $architecture == "s390x" ]]; then
      ...
      mongo_op_scaled_original=$(${OC} get deploy -n $ORIGINAL_NAMESPACE | grep ibm-mongodb-operator | grep -E '1/1' || echo false)
      ...
      mongo_op_scaled_target=$(${OC} get deploy -n $TARGET_NAMESPACE | grep ibm-mongodb-operator | grep -E '1/1' || echo false)
