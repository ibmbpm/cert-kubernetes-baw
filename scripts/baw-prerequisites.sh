            ...
            case $item_pattern in
                ...
                "(a) Workflow Authoring")
                    if [[ $DEPLOYMENT_TYPE == "production" ]]; then
                        # Exclude BAI for EKS and AKS platforms
                        if [[ "$OTHER_PLATFORM_TYPE" == "eks" || "$OTHER_PLATFORM_TYPE" == "aks" ]]; then
                ...
                "Workflow Process Service Authoring")
                    if [[ $DEPLOYMENT_TYPE == "production" ]]; then
                        # Exclude BAI for EKS and AKS platforms
                        if [[ "$OTHER_PLATFORM_TYPE" == "eks" || "$OTHER_PLATFORM_TYPE" == "aks" ]]; then
                            optional_components_list=("Data Collector and Data Indexer" "Exposed Kafka Services" "Workplace Assistant" "(Preview) Authoring Assistant" "Workflow Runtime MCP Server")
                            optional_components_cr_list=("pfs" "kafka" "workplace_assistant" "workflow_assistant" "workflow_runtime_mcp_server")
                        else
...
function check_property_file(){
    ...
    if [[ $tmp_flag == "true" || $tmp_flag == "yes" || $tmp_flag == "y" ]]; then
        ...
        if [[ -n "$im_external_db_cert_folder" ]]; then
    ...
    if [[ $tmp_flag == "true" || $tmp_flag == "yes" || $tmp_flag == "y" ]]; then
        ...
        if [[ -n "$zen_external_db_cert_folder" ]]; then
    ...
    # if [[ "$EXTERNAL_POSTGRESDB_FOR_BTS" == "true" && ( " ${optional_component_cr_arr[@]} " =~ " bai " || " ${current_cr_optional_components_array[@]} " =~ " bai " ) ]]; then
        if [[ $tmp_flag == "true" || $tmp_flag == "yes" || $tmp_flag == "y" ]]; then
            ...
            if [[ -n "$bts_external_db_cert_folder" ]]; then
    ...
    if [[ $tmp_flag == "true" || $tmp_flag == "yes" || $tmp_flag == "y" ]]; then
        ...
        if [[ -n "$external_cert_issuer_folder" ]]; then
    ...
    for item in "${db_server_array[@]}"; do
        ...
        for i in "${!GCDDB_COMMON_PROPERTY[@]}"; do
            if [[ ($DB_TYPE == "db2"*) && ${GCDDB_COMMON_PROPERTY[i]} != "GCD_DB_NAME" && ${GCDDB_COMMON_PROPERTY[i]} != "GCD_DB_USER_NAME" && ${GCDDB_COMMON_PROPERTY[i]} != "ORACLE_JDBC_URL" ]]; then
                ...
                echo "$item.${GCDDB_COMMON_PROPERTY[i]}=\"\"" >> ${DB_SERVER_INFO_PROPERTY_FILE}
                echo "" >> "${DB_SERVER_INFO_PROPERTY_FILE}"
            elif [[ ($DB_TYPE == "postgresql"* || $DB_TYPE == "sqlserver" || $DB_TYPE == "azuresqlmi") && ${GCDDB_COMMON_PROPERTY[i]} != "GCD_DB_NAME" && ${GCDDB_COMMON_PROPERTY[i]} != "GCD_DB_USER_NAME" && ${GCDDB_COMMON_PROPERTY[i]} != "HADR_STANDBY_SERVERNAME" && ${GCDDB_COMMON_PROPERTY[i]} != "HADR_STANDBY_PORT" && ${GCDDB_COMMON_PROPERTY[i]} != "ORACLE_JDBC_URL" ]]; then
                echo "${GCDDB_PROPERTY_COMMENTS[i]}" >> ${DB_SERVER_INFO_PROPERTY_FILE}
                echo "$item.${GCDDB_COMMON_PROPERTY[i]}=\"\"" >> ${DB_SERVER_INFO_PROPERTY_FILE}
                echo "" >> "${DB_SERVER_INFO_PROPERTY_FILE}"
            fi
...
\\${nl}" ${DB_SERVER_INFO_PROPERTY_FILE}
    ...
    if [[ " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workflow-authoring" || " ${pattern_cr_arr[@]}" =~ "workstreams" || " ${pattern_cr_arr[@]}" =~ "content" || " ${pattern_cr_arr[@]}" =~ "document_processing" || "${optional_component_cr_arr[@]}" =~ "ae_data_persistence" ]]; then
        ...
        if [[ " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workflow-authoring" ]]; then
            ...
            elif [[ $DB_TYPE == "sqlserver" || $DB_TYPE == "azuresqlmi" ]]; then
    ...
    if [[ "${optional_component_cr_arr[@]}" =~ "workflow_assistant" || "${optional_component_cr_arr[@]}" =~ "workplace_assistant" ]]; then
        ...
        # Generate deployment ID parameters based on environment type
        if [[ "${pattern_cr_arr[@]}" =~ "workflow-authoring" ]]; then
            # Authoring environments can test both authoring and runtime agents
            echo "## Optional - Only required if a custom deployment LLM model is being used for the authoring agent." >> ${USER_PROFILE_PROPERTY_FILE}
            echo "WFA.AUTHORING_AGENT_WATSONX_DEPLOYMENT_ID=\"<Optional>\"" >> ${USER_PROFILE_PROPERTY_FILE}
            echo "" >> ${USER_PROFILE_PROPERTY_FILE}

            echo "## Optional - Only required if a custom deployment LLM model is being used for the runtime agent." >> ${USER_PROFILE_PROPERTY_FILE}
            ...
            # Runtime environments only have runtime agent
            echo "## Optional - Only required if a custom deployment LLM model is being used for the runtime agent." >> ${USER_PROFILE_PROPERTY_FILE}
            echo "WFA.RUNTIME_AGENT_WATSONX_DEPLOYMENT_ID=\"<Optional>\"" >> ${USER_PROFILE_PROPERTY_FILE}
            echo "" >> ${USER_PROFILE_PROPERTY_FILE}
        fi
...
function create_db_script(){
    ...
    if [[ " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workflow-authoring" || " ${pattern_cr_arr[@]}" =~ "workstreams" || " ${pattern_cr_arr[@]}" =~ "content" || " ${pattern_cr_arr[@]}" =~ "document_processing" || "${optional_component_cr_arr[@]}" =~ "ae_data_persistence" ]]; then
        ...
        while true; do
            case "$DB_TYPE" in
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                tmp_dbname="$(prop_db_name_user_property_file GCD_DB_NAME)"
                ...
                elif [[ $DB_TYPE == "azuresqlmi" ]]; then
                    create_fncm_gcddb_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
            ...
            while true; do
                ...
                "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                    ...
                    if [[ $DB_TYPE == "sqlserver" ]]; then
                        ...
                        create_fncm_osdb_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername ${j} "" "$tmp_table_storage_location" "$tmp_index_storage_location" "$tmp_lob_storage_location"
                        # remove db_securityadmin/bulkadmin
                        ${SED_COMMAND} '/db_securityadmin/d' $FNCM_DB_SCRIPT_FOLDER/$DB_TYPE/$tmp_dbservername/createOS${j}DB.sql
                        ${SED_COMMAND} '/bulkadmin/d' $FNCM_DB_SCRIPT_FOLDER/$DB_TYPE/$tmp_dbservername/createOS${j}DB.sql
                    elif [[ $DB_TYPE == "postgresql" ]]; then
            ...
            while true; do
                ...
                "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                    ...
                    if [[ $DB_TYPE == "sqlserver" ]]; then
                        ...
                        create_ban_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                    elif [[ $DB_TYPE == "postgresql" ]]; then
        ...
        while true; do
            ...
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                ...
                if [[ $DB_TYPE == "sqlserver" ]]; then
                    ...
                    create_odm_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                elif [[ $DB_TYPE == "postgresql" ]]; then
                ...
                if [[ " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workflow-authoring" ]]; then
                    for i in "${!BAW_AUTH_OS_ARR[@]}"; do
                        ...
                        if [[ $DB_TYPE == "sqlserver" ]]; then
                            ...
                            if [[ "${BAW_AUTH_OS_ARR[i]}" == "BAWTOS" ]]; then
                                tmp_tablespace=$(prop_user_profile_property_file CONTENT_INITIALIZATION.CPE_OBJ_STORE_WORKFLOW_DATA_TBL_SPACE)
                                create_fncm_osdb_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername "" "$tmp_tablespace" "$tmp_table_storage_location" "$tmp_index_storage_location" "$tmp_lob_storage_location"
                            else
                                create_fncm_osdb_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername "" "" "$tmp_table_storage_location" "$tmp_index_storage_location" "$tmp_lob_storage_location"
                            fi
                    ...
                    if [[ $tmp_dbservername != \#* ]] ; then
                        ...
                        if [[ $DB_TYPE == "sqlserver" ]]; then
                            ...
                            create_fncm_osdb_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                            # remove db_securityadmin/bulkadmin
                            ${SED_COMMAND} '/db_securityadmin/d' $FNCM_DB_SCRIPT_FOLDER/$DB_TYPE/$tmp_dbservername/create$tmp_dbname.sql
                            ${SED_COMMAND} '/bulkadmin/d' $FNCM_DB_SCRIPT_FOLDER/$DB_TYPE/$tmp_dbservername/create$tmp_dbname.sql
                        elif [[ $DB_TYPE == "postgresql" ]]; then
            ...
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                if [[ " ${pattern_cr_arr[@]}" =~ "workflow-workstreams" ]]; then
                    ...
                    if [[ $DB_TYPE == "sqlserver" ]]; then
                        ...
                        create_bawaws1_db_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                    elif [[ $DB_TYPE == "postgresql" ]]; then
                    ...
                    if [[ $DB_TYPE == "sqlserver" ]]; then
                        ...
                        create_bawaws2_db_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                    elif [[ $DB_TYPE == "postgresql" ]]; then
        ...
        while true; do
            ...
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                ...
                if [[ $DB_TYPE == "sqlserver" ]]; then
                    ...
                    create_bas_studio_db_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                elif [[ $DB_TYPE == "postgresql" ]]; then
        ...
        while true; do
            ...
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                ...
                if [[ $DB_TYPE == "sqlserver" ]]; then
                    ...
                    create_ae_playback_db_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                elif [[ $DB_TYPE == "postgresql" ]]; then
        ...
        while true; do
            ...
            "db2"|"sqlserver"|"azuresqlmi"|"postgresql")
                ...
                if [[ $DB_TYPE == "sqlserver" ]]; then
                    ...
                    create_baa_app_engine_db_azuresqlmi_sql_file $tmp_dbname $tmp_dbuser $tmp_dbuserpwd $tmp_dbservername
                elif [[ $DB_TYPE == "postgresql" ]]; then
...
function select_db_type(){
    ...
    # Set PS3 based on the actual number of options
...
function input_information(){
    ...
    if  [[ $PLATFORM_SELECTED == "OCP" || $PLATFORM_SELECTED == "ROKS" || $PLATFORM_SELECTED == "other" ]]; then
        ...
        ### User is asked whether to use external postgres for IM/Zen (select_external_postgresdb_for_im_zen handles skip logic for 25.0.1)
...
function select_platform(){
    ...
    # DBACLD-168151
    otheroption="Other ( Rancher Kubernetes Engine (RKE) / VMware Tanzu Kubernetes Grid Integrated Edition (TKGI) / Google Kubernetes Engine (GKE) / Amazon Elastic Kubernetes Service (EKS) / Azure Kubernetes Service (AKS))"
    ...
    do
        case $opt in
            ...
            "$otheroption")
                ...
                # Sub-menu to select specific "other" platform type
