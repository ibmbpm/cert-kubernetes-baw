...
function generate_cncf_ingress_templates(){
    ...
    printf "\n"
    echo "${RED_TEXT}[WARNING]${RESET_TEXT}: ${YELLOW_TEXT}Before proceeding with the ingress generation, make sure the BAW Standalone components have been installed by looking at the status section of the InsightsEngine Custom Resource file and monitoring the Insights Engine Operator logs ${RESET_TEXT}"
    attempt=0
