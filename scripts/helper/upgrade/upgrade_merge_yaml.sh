...
function migrate_watsonx_deployment_id() {
    local input_yaml="$1"
    
    # Check if old watsonx_deployment_id exists
    old_deployment_id=$(${YQ_CMD} r "$input_yaml" "spec.workflow_assistant_configuration.watsonx_deployment_id")
    ...
    if [[ -n "$old_deployment_id" && "$old_deployment_id" != "null" ]]; then
        ...
        # Authoring environments have 'baw_authoring' in sc_optional_components
        sc_optional_components=$(${YQ_CMD} r "$input_yaml" "spec.shared_configuration.sc_optional_components")
        
        is_authoring=false
        
        # Check if sc_optional_components contains baw_authoring
        if [[ "$sc_optional_components" == *"baw_authoring"* ]]; then
            is_authoring=true
        fi
        ...
        if [[ "$is_authoring" == "true" ]]; then
            ...
            info "Detected authoring environment (baw_authoring) - setting both authoring_agent_watsonx_deployment_id and runtime_agent_watsonx_deployment_id"
            ${YQ_CMD} w -i "$input_yaml" "spec.workflow_assistant_configuration.authoring_agent_watsonx_deployment_id" "$old_deployment_id"
            ${YQ_CMD} w -i "$input_yaml" "spec.workflow_assistant_configuration.runtime_agent_watsonx_deployment_id" "$old_deployment_id"
        else
...
function upgrade_deployment(){
    ...
    if [ $? -eq 0 ]; then
        content_cr_name=$(${CLI_CMD} get content.icp4a.ibm.com -n $deployment_project_name --no-headers --ignore-not-found | awk '{print $1}')
        if [ ! -z $content_cr_name ]; then
            info "Retrieving existing CP4BA Content (Kind: content.icp4a.ibm.com) Custom Resource"
            cr_type="content"
            cr_metaname=$(${CLI_CMD} get content.icp4a.ibm.com $content_cr_name -n $deployment_project_name -o yaml | ${YQ_CMD} r - metadata.name)
            cr_version=$(${CLI_CMD} get content.icp4a.ibm.com $content_cr_name -n $deployment_project_name -o yaml | ${YQ_CMD} r - spec.appVersion)
            owner_ref=$(${CLI_CMD} get content.icp4a.ibm.com $content_cr_name -n $deployment_project_name -o yaml | ${YQ_CMD} r - metadata.ownerReferences.[0].kind)
            if [[ ${owner_ref} == "ICP4ACluster" ]]; then
                ...
                ${YQ_CMD} w -i ${UPGRADE_DEPLOYMENT_CONTENT_CR_TMP} spec.shared_configuration.sc_enable_usage_metering "true"
                ...
                if [[ $APPLY_UPDATED_CR == "Yes" ]]; then
                    ...
                    echo "  STEP1 ${RED_TEXT}(Required)${RESET_TEXT}:${GREEN_TEXT} # ./baw-deployment.sh -m upgradeDeploymentStatus -n $TARGET_PROJECT_NAME${RESET_TEXT}"
                else

                    initialize_cfg_flag=$(${CLI_CMD} get content.icp4a.ibm.com $content_cr_name -n $deployment_project_name --no-headers --ignore-not-found -o 'jsonpath={.spec.initialize_configuration}') >/dev/null 2>&1
                    verify_cfg_flag=$(${CLI_CMD} get content.icp4a.ibm.com $content_cr_name -n $deployment_project_name --no-headers --ignore-not-found -o 'jsonpath={.spec.verify_configuration}') >/dev/null 2>&1
                    ...
                    echo "  - STEP ${step_num} ${RED_TEXT}(Required)${RESET_TEXT}: ${GREEN_TEXT}# ./baw-deployment.sh -m upgradeDeploymentStatus -n $TARGET_PROJECT_NAME${RESET_TEXT}"
                fi
                printf "\n"
                ...
                #     printf "\n"
                #     # echo "${YELLOW_TEXT}[NEXT ACTION]${RESET_TEXT}: "
                #     # msgB "Run \"baw-deployment.sh -m upgradeDeploymentStatus -n $deployment_project_name\" to get overview upgrade status for CP4BA"
                # fi
            fi
        fi
    fi
    ...
    if [ ! -z $icp4acluster_cr_name ]; then
        ...
        # for BAW authoring, set workflow_authoring_configuration.case.tos_list
        # Support multiple tos instance from $CP4BA_RELEASE_BASE
        # DBACLD-177124: Updated the logic to dynamically check if this is an ifix to ifix upgrade or not.  If not, then we'll set the is_baw_cr_updated_needed to true
        ...
        # for BAW runtime, set baw_configuration.case.tos_list
        # Support multiple tos instance from $CP4BA_RELEASE_BASE
        # DBACLD-177124: Updated the logic to dynamically check the version against the MINIMUM_SUPPORTED_UPGRADE_VERSIONS
        # This condition is only needed for n-1 to n upgrade
        if [[ $cr_version != "${CP4BA_RELEASE_BASE}" && $is_baw_cr_updated_needed == true ]]; then
            if [[ (! " ${EXISTING_OPT_COMPONENT_ARR[@]} " =~ "baw_authoring") && (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow" || " ${EXISTING_PATTERN_ARR[@]} " =~ "workflow-workstreams") ]]; then
                # Support multiple tos instance from $CP4BA_RELEASE_BASE
        ...
        if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "content") || (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow") || (" ${EXISTING_PATTERN_ARR[@]} " =~ "document_processing") || (" ${EXISTING_OPT_COMPONENT_ARR[@]} " =~ "baw_authoring") || (" ${EXISTING_OPT_COMPONENT_ARR[@]} " =~ "ae_data_persistence") ]]; then
            ...
            # By default we will always set the sc_enable_usage_metering flag to true for ICP4ACluster CR
            # https://jsw.ibm.com/browse/DBACLD-225399
            ${YQ_CMD} w -i ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR_TMP} spec.shared_configuration.sc_enable_usage_metering "true"
        ...
        success "Completed to merge existing BAW Custom Resource with new version ($CP4BA_RELEASE_BASE)"
        ...
        info "The new version ($CP4BA_RELEASE_BASE) of BAW Custom Resource is created ${UPGRADE_DEPLOYMENT_ICP4ACLUSTER_CR}"
        ...
        if [[ $APPLY_UPDATED_CR == "Yes" ]]; then
            ...
            echo "${YELLOW_TEXT}[NEXT ACTION]:${RESET_TEXT}"
            echo "${YELLOW_TEXT}- How to check the overall upgrade status for BAW/zenService/IM.${RESET_TEXT}"
            echo "${YELLOW_TEXT}  [TIPS]: ${RESET_TEXT}The [upgradeDeploymentStatus] option will start necessary BAW operators (ibm-cp4a-operator/icp4a-foundation-operator) first to upgrade zenService, and then will start all other BAW operators when zenService upgrade done."
            echo "  STEP1 ${RED_TEXT}(Required)${RESET_TEXT}:${GREEN_TEXT} # ./baw-deployment.sh -m upgradeDeploymentStatus -n $TARGET_PROJECT_NAME${RESET_TEXT}"
        else
        ...
        printf "\n"
        echo "${YELLOW_TEXT}[ATTENTION]: The zenService will be ready in about 120 minutes after the new version ($CP4BA_RELEASE_BASE) of BAW custom resource was applied.${RESET_TEXT}"
        printf "\n"
