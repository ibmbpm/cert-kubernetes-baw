...
isInstalled=`cat ${UPGRADE_DEPLOYMENT_WFPSRUNTIME_CR_TMP}| ${YQ_CMD} r - status.components.wfps.wfpsDeployment`
...
isInstalled=`cat ${UPGRADE_DEPLOYMENT_WFPSRUNTIME_CR_TMP}| ${YQ_CMD} r - status.components.wfps.wfpsService`
if [ "$isInstalled" == "NotInstalled" ]; then
...
printHeaderMessage "BAW on Containers Upgrade Status - WfPS(instance: $wfps_cr_metaname)"
