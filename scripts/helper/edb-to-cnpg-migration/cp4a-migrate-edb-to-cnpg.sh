...
function create_edb_cnpg_migration_configmap() {
    ...
    cat <<EOF | ${CLI_CMD} apply -f - >/dev/null 2>&1
apiVersion: v1
kind: ConfigMap
metadata:
...
function get_migration_phase_status() {
    ...
    if ! ${CLI_CMD} get configmap ${EDB_CNPG_MIGRATION_CM_NAME} -n $namespace >/dev/null 2>&1; then
        echo "false"
        return 0
    fi
...
function update_migration_phase_status() {
    ...
    ${CLI_CMD} patch configmap ${EDB_CNPG_MIGRATION_CM_NAME} -n $namespace \
        --type merge \
        -p "{\"data\":{\"$phase\":\"$status\",\"$phase-timestamp\":\"$(date -u +"%Y-%m-%dT%H:%M:%SZ")\"}}" >/dev/null 2>&1
...
function check_migration_configmap_exists() {
    ...
    ${CLI_CMD} get configmap ${EDB_CNPG_MIGRATION_CM_NAME} -n $namespace >/dev/null 2>&1
    return $?
}
        ...
        backup)
            ...
            if execute_backup_phase "$services_namespace" "$operator_namespace" "$backup_dir" "$cr_kind"; then
                ...
                ${CLI_CMD} patch configmap ${EDB_CNPG_MIGRATION_CM_NAME} -n $services_namespace \
                    --type merge \
                    -p "{\"data\":{\"backup-directory\":\"$backup_dir\"}}" >/dev/null 2>&1
...
function scale_down_applications() {
    ...
    for dep in "${deployments_to_scale[@]}"; do
        ...
        if ${CLI_CMD} scale deployment "$dep" -n $SERVICES_NAMESPACE --replicas=0 >/dev/null 2>&1; then
...
function backup_databases() {
    ...
    ${CLI_CMD} exec -n $SERVICES_NAMESPACE $EDB_POD -- psql -U postgres -t -A -c \
        "SELECT 'GRANT ' || privilege_type || ' ON DATABASE ' || quote_ident(db) || ' TO ' || quote_ident(grantee) || ';'
         FROM (
             SELECT d.datname AS db,
                    (aclexplode(d.datacl)).privilege_type AS privilege_type,
                    (aclexplode(d.datacl)).grantee::regrole::text AS grantee
             FROM pg_database d
             WHERE d.datistemplate = false
               AND d.datname NOT IN ('postgres', 'rdsadmin')
    ...
    # Fix for CNPG incompatibilities in globals.sql and db_ownership_grants.sql
    if [[ -f "$BACKUP_DIR/globals.sql" ]]; then
        info "Filtering out CREATE TABLESPACE commands from globals backup..."
        if [[ "$(uname -s)" == "Darwin" ]]; then
            sed -i '' '/CREATE TABLESPACE/d' "$BACKUP_DIR/globals.sql"
        else
            sed -i '/CREATE TABLESPACE/d' "$BACKUP_DIR/globals.sql"
        fi
    fi
    ...
    if [[ -f "$BACKUP_DIR/db_ownership_grants.sql" ]] && [[ -s "$BACKUP_DIR/db_ownership_grants.sql" ]]; then
        info "Filtering out invalid role references from ownership grants..."
        if [[ "$(uname -s)" == "Darwin" ]]; then
            sed -i '' '/role "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
            sed -i '' '/TO "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
        else
            sed -i '/role "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
            sed -i '/TO "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
        fi
...
function install_cnpg_operator() {
    ...
    if ${CLI_CMD} get subscription ibm-pg-operator -n $OPERATOR_NAMESPACE &> /dev/null; then
        ...
        ${YQ_CMD} w -i "$CNPG_SUBSCRIPTION_MANIFEST" metadata.namespace "$OPERATOR_NAMESPACE"
        ${YQ_CMD} w -i "$CNPG_SUBSCRIPTION_MANIFEST" spec.sourceNamespace "$OPERATOR_NAMESPACE"
        
        # Replace channel placeholder
        ${YQ_CMD} w -i "$CNPG_SUBSCRIPTION_MANIFEST" spec.channel "$CNPG_OPERATOR_CHANNEL"
...
function generate_cnpg_manifest() {
    ...
    ${YQ_CMD} w -i "$CNPG_MANIFEST" metadata.name "${NEW_CLUSTER}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" metadata.namespace "${SERVICES_NAMESPACE}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.instances "${instances}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.storage.size "${storage_size}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.storage.storageClass "${storage_class}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.resources.requests.memory "${memory_requests}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.resources.requests.cpu "${cpu_requests}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.resources.limits.memory "${memory_limits}"
    ${YQ_CMD} w -i "$CNPG_MANIFEST" spec.resources.limits.cpu "${cpu_limits}"
...
function restore_databases() {
    ...
    # Ensure fixes for CNPG incompatibilities in globals.sql and db_ownership_grants.sql 
    # are applied before copying to pod (in case they were not applied during backup)
    if [[ -f "$BACKUP_DIR/globals.sql" ]]; then
        info "Filtering out CREATE TABLESPACE commands from globals backup before restore..."
        if [[ "$(uname -s)" == "Darwin" ]]; then
            sed -i '' '/CREATE TABLESPACE/d' "$BACKUP_DIR/globals.sql"
        else
            sed -i '/CREATE TABLESPACE/d' "$BACKUP_DIR/globals.sql"
    ...
    if [[ -f "$BACKUP_DIR/db_ownership_grants.sql" ]] && [[ -s "$BACKUP_DIR/db_ownership_grants.sql" ]]; then
        info "Filtering out invalid role references from ownership grants before restore..."
        if [[ "$(uname -s)" == "Darwin" ]]; then
            sed -i '' '/role "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
            sed -i '' '/TO "-"/d' "$BACKUP_DIR/db_ownership_grants.sql"
        else
    ...
    for db in "${DATABASES[@]}"; do
        ...
        # -d uses the exact name as a connection string; quoting the shell var is sufficient
