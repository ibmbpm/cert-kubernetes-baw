...
function catalog_source_staging() {
    ...
    for name in "${catalog_names[@]}"; do
        ...
        if [[ "$RUNTIME_MODE" == "dev" ]]; then
            ...
            if [[ "$name" == "ibm-cp4a-operator-catalog" || "$name" == "ibm-fncm-operator-catalog"  || "$name" == "ibm-zen-operator-catalog-6-4-0" || "$name" == "cloud-native-postgresql-catalog" || "$name" == "ibm-cp-automation-catalog" ]]; then
