...
LATEST_REGISTRY_IN_FILE="icr.io"
...
function select_private_catalog(){
    # For offline/airgap deployments, skip the prompt and automatically set private catalog to Yes
    # This is required because airgap deployments use private registries and need namespace-scoped catalogs
    if [[ $AIRGAP_INSTALL == "yes" ]]; then
        PRIVATE_CATALOG="Yes"
        printf "\n"
        echo "${YELLOW_TEXT}[INFO] For offline/airgap deployment, private catalog (namespace scope) is automatically enabled.${RESET_TEXT}"
        return 0
    fi
...
function create_common_service_configmap(){
    ...
    # This was introduced because of the RBAC changes we made in 26.0.0
...
function apply_cp4a_operator(){
    ...
    # For latest mode, transform operator images with :latest tag
    # Latest mode works on top of dev mode: catalog sources use cp.stg.icr.io (dev),
    # but operator deployment images are patched to use icr.io with :latest tags
    # icp4a-operator uses icr.io/cp4ba-operate/operators
    # workflow-ps-operator, workflow-runtime-operator, pfs-operator use icr.io/cp4ba-baw
    if [[ "$RUNTIME_MODE" == "latest" ]]; then
        # Transform icp4a-operator images (both init container and main container)
        ${SED_COMMAND} 's|image: icr.io/cpopen/icp4a-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-operate/operators/icp4a-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/icp4a-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-operate/operators/icp4a-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: icr.io/cpopen/icp4a-operator:[^[:space:]]*|image: icr.io/cp4ba-operate/operators/icp4a-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/icp4a-operator:[^[:space:]]*|image: icr.io/cp4ba-operate/operators/icp4a-operator:latest|g' ${OPERATOR_FILE_TMP}
        
        # Transform workflow-ps-operator images (use cp4ba-baw)
        ${SED_COMMAND} 's|image: icr.io/cpopen/workflow-ps-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/workflow-ps-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/workflow-ps-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/workflow-ps-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: icr.io/cpopen/workflow-ps-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/workflow-ps-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/workflow-ps-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/workflow-ps-operator:latest|g' ${OPERATOR_FILE_TMP}
        
        # Transform workflow-runtime-operator images (use cp4ba-baw)
        ${SED_COMMAND} 's|image: icr.io/cpopen/workflow-runtime-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/workflow-runtime-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/workflow-runtime-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/workflow-runtime-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: icr.io/cpopen/workflow-runtime-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/workflow-runtime-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/workflow-runtime-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/workflow-runtime-operator:latest|g' ${OPERATOR_FILE_TMP}
        
        # Transform pfs-operator images (use cp4ba-baw)
        ${SED_COMMAND} 's|image: icr.io/cpopen/pfs-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/pfs-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/pfs-operator@sha256:[a-f0-9]*|image: icr.io/cp4ba-baw/pfs-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: icr.io/cpopen/pfs-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/pfs-operator:latest|g' ${OPERATOR_FILE_TMP}
        ${SED_COMMAND} 's|image: cp.icr.io/cp/cp4a/pfs-operator:[^[:space:]]*|image: icr.io/cp4ba-baw/pfs-operator:latest|g' ${OPERATOR_FILE_TMP}
    fi
...
function wait_for_pods_active() {
    ...
    if [[ $RUNTIME_MODE == "dev" || $RUNTIME_MODE == "latest" ]];then
        ...
        patch_csv "ibm-ads-operator" $project_name
        patch_csv "ibm-ccx-ai-services-operator" $project_name
    fi
...
function catalog_source_staging() {
    ...
    for name in "${catalog_names[@]}"; do
        ...
        if [[ "$RUNTIME_MODE" == "dev" || "$RUNTIME_MODE" == "latest" ]]; then
            ...
            if [[ "$name" == "ibm-cp4a-operator-catalog" || "$name" == "ibm-content-cortex-operator-catalog"  || "$name" == "ibm-zen-operator-catalog-6-4-0" || "$name" == "cloud-native-postgresql-catalog" || "$name" == "ibm-cp-automation-catalog" || "$name" == "ibm-redis-cp-operator-catalog" ]]; then
                ...
                if [[ -n "$current_image" && "$current_image" == icr.io/cpopen/* ]]; then
                    # Modify the repository path to use staging registry (dev mode base)
        ...
        # In latest mode, catalog sources use dev mode (cp.stg.icr.io)
        # Operator deployment images are transformed to :latest (handled in apply_cp4a_operator function)
...
function verify_entitlement_key() {
  ...
  while [[ -z $entitlement_key ]]; do
    ...
    elif [[ "$USER_DOCKER_REG_SERVER" == "icr.io" ]]; then
      if [[ -z "$BAW_AUTO_LATEST_ENTITLEMENT_KEY" ]]; then
        read -rsp " " entitlement_key
        echo
      else
        entitlement_key=$BAW_AUTO_LATEST_ENTITLEMENT_KEY
      fi
    else
    ...
    if [[ -z "$entitlement_key" ]]; then
      ...
      if [[ $entitlement_key == iamapikey:* ]]; then
        ...
        # Also handle latest registry if we're using it
        if [[ "$USER_DOCKER_REG_SERVER" == "icr.io" ]]; then
           DOCKER_REG_USER_LATEST="iamapikey"
           DOCKER_REG_KEY_LATEST="${entitlement_key#*:}"
        fi
      else
        if [[ "$USER_DOCKER_REG_SERVER" == "cp.stg.icr.io" ]]; then
           DOCKER_REG_KEY_STG=$entitlement_key
           ...
           # For icr.io, username must always be iamapikey
           DOCKER_REG_KEY_LATEST=$entitlement_key
           DOCKER_REG_USER_LATEST="iamapikey"
           DOCKER_REG_USER="iamapikey"
           reg_key=$entitlement_key
...
function get_entitlement_registry(){
    ...
    if [[ "$RUNTIME_MODE" == "latest" ]]
    then
        # Latest mode works on top of dev mode, so we need credentials for all three registries
        # Get staging key for catalog sources (dev mode base)
        get_stg_entitlement_registry
        
        # Set DOCKER_REG_SERVER to cp.icr.io and use the same credentials as staging
        # (cp.icr.io and cp.stg.icr.io use the same entitlement key)
        DOCKER_REG_SERVER="cp.icr.io"
        DOCKER_REG_USER="$DOCKER_REG_USER_STG"
        DOCKER_REG_KEY="$DOCKER_REG_KEY_STG"
        
        # Then get latest key for patching operator images to :latest
        get_latest_entitlement_registry
    fi
...
# Function that asks for the latest entitlement key and verifies it
# For latest mode it verifies against icr.io
function get_latest_entitlement_registry(){

    # For Entitlement Registry key
    entitlement_key=""

    # If env vars are set, skip interactive prompts
    USER_DOCKER_REG_SERVER="icr.io"
    if [ -n "$BAW_AUTO_LATEST_ENTITLEMENT_KEY" ]; then
        # For icr.io, username must always be iamapikey
        # Check if the key starts with iamapikey: and handle accordingly
        if [[ "$BAW_AUTO_LATEST_ENTITLEMENT_KEY" == iamapikey:* ]]; then
            DOCKER_REG_USER_LATEST="iamapikey"
            DOCKER_REG_KEY_LATEST="${BAW_AUTO_LATEST_ENTITLEMENT_KEY#*:}"
            DOCKER_REG_KEY="${BAW_AUTO_LATEST_ENTITLEMENT_KEY#*:}"
        else
            # Assume the provided key is just the API key without prefix
            DOCKER_REG_USER_LATEST="iamapikey"
            DOCKER_REG_KEY_LATEST="$BAW_AUTO_LATEST_ENTITLEMENT_KEY"
            DOCKER_REG_KEY="$BAW_AUTO_LATEST_ENTITLEMENT_KEY"
        fi
        echo -e "\x1B[1mUsing latest entitlement key from env var.\x1B[0m"
        verify_entitlement_key $USER_DOCKER_REG_SERVER
        use_entitlement="yes"
        return 0
    fi
    ...
    while true; do
        ...
        printf "\x1B[1mDo you have the ICCR keys (Yes/No, default: No): \x1B[0m"
        read -rp "" ans
        ...
        "y"|"Y"|"yes"|"Yes"|"YES")
            use_entitlement="yes"
            printf "\n"
            printf "\x1B[1mEnter your ICCR key: \x1B[0m"
            verify_entitlement_key $USER_DOCKER_REG_SERVER
            break
            ;;
        "n"|"N"|"no"|"No"|"NO"|"")
...
function create_secret_entitlement_registry(){
    ...
    if [[ $SEPARATE_OPERATOR == "No" || -z $SEPARATE_OPERATOR ]]; then
        ...
        then
            if [[ $AIRGAP_INSTALL != "Yes" ]]; then
                # Latest mode needs all three registry credentials: cp.icr.io (production), cp.stg.icr.io (staging), and icr.io (latest)
                # Use merged secret that contains credentials for all three registries
                create_ibm_merged_stg_latest_entitlement_key
            fi
        fi
    else
        ...
        then
            if [[ $AIRGAP_INSTALL != "Yes" ]]; then
                # Latest mode needs all three registry credentials: cp.icr.io (production), cp.stg.icr.io (staging), and icr.io (latest)
                # Use merged secret that contains credentials for all three registries
                printf "\x1B[1mCreating merged docker-registry secret (cp.icr.io + cp.stg.icr.io + icr.io) in project $project_name_operator...\n\x1B[0m"
                create_ibm_merged_stg_latest_entitlement_key "$project_name_operator"
            fi
        fi
        ...
        then
            if [[ $AIRGAP_INSTALL != "Yes" ]]; then
                # Latest mode needs all three registry credentials: cp.icr.io (production), cp.stg.icr.io (staging), and icr.io (latest)
                # Use merged secret that contains credentials for all three registries
                printf "\x1B[1mCreating merged docker-registry secret (cp.icr.io + cp.stg.icr.io + icr.io) in project $project_name_cs_service...\n\x1B[0m"
                create_ibm_merged_stg_latest_entitlement_key "$project_name_cs_service"
            fi
        fi
    fi
        ...
        for item in "${project_baw_service_array[@]}"; do
            ...
            then
                if [[ $AIRGAP_INSTALL != "Yes" ]]; then
                    # Latest mode needs all three registry credentials: cp.icr.io (production), cp.stg.icr.io (staging), and icr.io (latest)
                    # Use merged secret that contains credentials for all three registries
                    printf "\x1B[1mCreating merged docker-registry secret (cp.icr.io + cp.stg.icr.io + icr.io) in project $item...\n\x1B[0m"

                    create_ibm_merged_stg_latest_entitlement_key "$item"
                fi
            fi
        done
    fi
...
function check_airgap_mode(){
    ...
    if [ -z "$BAW_AUTO_AIRGAP_MODE" ]; then
        ...
        AIRGAP_INSTALL=$(echo "$BAW_AUTO_AIRGAP_MODE" | tr '[:upper:]' '[:lower:]')
...
function select_platform(){
    ...
    if [ -z "$BAW_AUTO_PLATFORM" ]; then
        ...
        # DBACLD-168151
        otherOption="Other ( Rancher Kubernetes Engine (RKE) / VMware Tanzu Kubernetes Grid Integrated Edition (TKGI) / Google Kubernetes Engine (GKE) / Amazon Elastic Kubernetes Service (EKS) / Azure Kubermetes Service (AKS))"
...
# Function to retrieve and save fsGroup for GKE/Rancher/Tanzu deployments
# DBACLD-168151
function retrieve_fsgroup() {
    local attempts=0
    local max_attempts=3
    local fsgroup_value=""

    # Check if FSGROUP already exists in environment
    if [[ -n "$FSGROUP" ]]; then
        success "Using fsGroup from environment: '$FSGROUP'"
        echo "FSGROUP=\"$FSGROUP\"" > ${TEMP_FOLDER}/baw-fsgroup.conf
        return 0
    fi

    # Prompt user for fsGroup input
    printf "\n"
    echo "${YELLOW_TEXT}[NOTES] For GKE, Rancher, and Tanzu deployments, you may need to specify an fsGroup value for proper file system permissions.${RESET_TEXT}"
    echo "${YELLOW_TEXT}[NOTES] Common values: GKE typically uses 1000, but this may vary based on your cluster configuration.${RESET_TEXT}"
    printf "\n"
    
    while [ $attempts -lt $max_attempts ]; do
        printf "\x1B[1mEnter the fsGroup value for your deployment (press Enter to skip if not required): \x1B[0m"
        read -rp "" fsgroup_value
        
        if [[ -z "$fsgroup_value" ]]; then
            info "No fsGroup value provided. Skipping fsGroup configuration."
            return 0
        elif [[ "$fsgroup_value" =~ ^[0-9]+$ ]]; then
            # Valid numeric value
            success "fsGroup value '$fsgroup_value' will be used for the deployment."
            echo "FSGROUP=\"$fsgroup_value\"" > ${TEMP_FOLDER}/baw-fsgroup.conf
            return 0
        else
            warning "Invalid fsGroup value. Please enter a numeric value."
        fi
    ...
    if [[ $attempts -eq $max_attempts ]]; then
        warning "Maximum number of retries exceeded. Proceeding without fsGroup configuration."
    fi
}


# Function that does the cluster setup for TANZU, Rancher, or GKE
# DBACLD-168151
function setup_other_type_platform()
{
    ...
    # Set CATALOG_NAMESPACE based on PRIVATE_CATALOG and RUNTIME_MODE
    # This must be set before calling install_ibm_usage_metering
    if [[ $PRIVATE_CATALOG == "Yes" ]]; then
        CATALOG_NAMESPACE=$project_name
    else
        CATALOG_NAMESPACE="openshift-marketplace"
    ...
    # $6 catalog_namespace which is the namespace where the catalog source is deployed (either project namespace or openshift-marketplace)
    ...
    # Add ILS Software Central configuration for VPC metrics
...
function create_ibm_latest_entitlement_key() {
    ${CLI_CMD} delete secret $DOCKER_RES_SECRET_NAME -n $project_name --ignore-not-found
    cat <<EOF > ${TEMP_FOLDER}/dockerconfig.json
    {
    "auths": {
    ...
    "$LATEST_REGISTRY_IN_FILE": {
    "username": "$DOCKER_REG_USER_LATEST",
    "password": "$DOCKER_REG_KEY_LATEST",
    "email": "ecmtest@ibm.com",
    "auth": "$(echo -n "$DOCKER_REG_USER_LATEST:$DOCKER_REG_KEY_LATEST" | base64 | tr -d '\n')"
    }
    }
    }
EOF
...
function create_ibm_merged_stg_latest_entitlement_key() {
    # For latest mode: create a single secret with credentials for ALL THREE registries: cp.icr.io, cp.stg.icr.io, and icr.io
    # Accept optional namespace parameter, default to $project_name
    local target_namespace=${1:-$project_name}
    
    ${CLI_CMD} delete secret $DOCKER_RES_SECRET_NAME -n $target_namespace --ignore-not-found
    cat <<EOF > ${TEMP_FOLDER}/dockerconfig.json
    {
    "auths": {
    "$DOCKER_REG_SERVER": {
...
EOF
    ...
    # Create the merged pull secret with all three registries
    CREATE_SECRET_CMD="${CLI_CMD} create secret generic $DOCKER_RES_SECRET_NAME  --from-file=.dockerconfigjson=${TEMP_FOLDER}/dockerconfig.json --type=kubernetes.io/dockerconfigjson -n $target_namespace"
...
if [[ ($PLATFORM_SELECTED == "OCP" || $PLATFORM_SELECTED == "ROKS") && $DEPLOYMENT_TYPE == "production" && $RUNTIME_MODE != "baw" && $RUNTIME_MODE != "baw-dev" && $RUNTIME_MODE != "latest" ]]; then
    check_fips_enable
fi
...
if [[ "$OTHER_PLATFORM_TYPE" == "rancher" ]]; then
    echo "${YELLOW_TEXT}[NOTES] If Rancher is deployed on RHEL and you plan to enable FIPS, follow the steps in https://ibmdocs-test.dcs.ibm.com/docs/en/baw_26.0.0_test?topic=overview-fips-compliance."
fi
...
if [[ "$OTHER_PLATFORM_TYPE" == "gke" ]]; then
    echo "${YELLOW_TEXT}[NOTES] FIPS is not supported on GKE."
fi

if [[ "$OTHER_PLATFORM_TYPE" == "eks" ]]; then
    echo "${YELLOW_TEXT}[NOTES] FIPS is not supported on EKS."
fi

if [[ "$OTHER_PLATFORM_TYPE" == "aks" ]]; then
    echo "${YELLOW_TEXT}[NOTES] FIPS is not supported on AKS."
fi

#Function that handles the platform type rancher, tanzu, gke, eks or aks
if [[ "$OTHER_PLATFORM_TYPE" == "rancher" || "$OTHER_PLATFORM_TYPE" == "tanzu" || "$OTHER_PLATFORM_TYPE" == "gke" || "$OTHER_PLATFORM_TYPE" == "eks" || "$OTHER_PLATFORM_TYPE" == "aks" ]]; then
...
# Function that handles all the usage metering operator related tasks
# The function will also be called during upgrade and hence has a mode argument i.e argument 3
# Function that handles all the usage metering operator related tasks
# At this point the UMS Set up would happen only for production
# https://jsw.ibm.com/browse/DBACLD-216413
# $1 Operator namespace
