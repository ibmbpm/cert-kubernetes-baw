#!/usr/bin/env bash
if [[ -z "${PREREQUISITES_FOLDER:-}" ]]; then
    source ./helper/common.sh "${TARGET_PROJECT_NAME:-$1}"
fi
...
function patch_csv() {
    ...
    function get_csv_by_prefix() {
        ...
        echo "$csv_list" | eval "$head_to_fetch"
    }
    ...
    for ((i=1; i<=max_retries; i++)); do
        ...
        if [[ "$i" -eq "$max_retries" ]]; then
            warning "CSV with prefix '$csv_prefix' not found after $max_retries attempts. Skipping patch for this operator..."
            return 0
        fi

        echo "CSV $csv_prefix not found. Retrying in $retry_delay seconds... ($i/$max_retries)"
        sleep "$retry_delay"
    done
    ...
    # Transform the image path based on runtime mode
    if [[ "$RUNTIME_MODE" == "latest" ]]; then
        # For latest mode: Only transform specific BAW operators with :latest tag
        ...
        if [[ "$csv_prefix" == "ibm-cp4a-operator" ]]; then
            # Extract the operator name from the image
            operator_name=$(echo "$image" | sed -E 's|.*/([^/:@]+)[@:].*|\1|')
            updated_image="icr.io/cp4ba-operate/operators/${operator_name}:latest"
        elif [[ "$csv_prefix" == "ibm-cp4a-wfps-operator" || "$csv_prefix" == "ibm-pfs-operator" || "$csv_prefix" == "ibm-workflow-operator" ]]; then
            # Extract the operator name from the image
            operator_name=$(echo "$image" | sed -E 's|.*/([^/:@]+)[@:].*|\1|')
            updated_image="icr.io/cp4ba-baw/${operator_name}:latest"
        else
            # For other operators in latest mode (including content-operator, odm-operator, etc.),
            # transform to staging registry same as dev mode
            updated_image=$(echo "$image" | sed -E 's|^icr.io/cpopen/|cp.stg.icr.io/cp/|')
        fi
    else
        # For dev mode: transform to cp.stg.icr.io/cp/
        updated_image=$(echo "$image" | sed -E 's|^icr.io/cpopen/|cp.stg.icr.io/cp/|')
    fi
    ...
    # Patch the containerImage annotation if it exists
    containerImageAnnotation=$(${CLI_CMD} get csv "$csv_name" -n "$namespace" -o jsonpath='{.metadata.annotations.containerImage}' 2>/dev/null)
    if [[ -n "$containerImageAnnotation" ]]; then
        updated_annotation=$(echo "$containerImageAnnotation" | sed -E 's|^icr.io/cpopen/|cp.stg.icr.io/cp/|')
        ${CLI_CMD} annotate csv "$csv_name" -n "$namespace" containerImage="$updated_annotation" --overwrite
    fi
    ...
    ${CLI_CMD} delete deployment $csv_prefix -n "$namespace"
...
function create_baw_subscription() {
    ...
    if [[ "$dev_mode" == true || "$RUNTIME_MODE" == "latest" ]]; then
        ...
        patch_csv "ibm-ads-operator" $namespace
        patch_csv "ibm-ccx-ai-services-operator" $namespace
    fi
